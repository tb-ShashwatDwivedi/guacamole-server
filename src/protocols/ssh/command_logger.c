...
command_logger* guac_ssh_command_logger_create(guac_user* user, const char* username,
    ...
    /* Use Guacamole's user_id as the unique connection ID */
    if (user->user_id && strlen(user->user_id) > 0) {
    ...
    if (username && strlen(username) > 0) {
        strncpy(logger->username, username, sizeof(logger->username) - 1);
        logger->username[sizeof(logger->username) - 1] = '\0';
    } else {
        strcpy(logger->username, "unknown");
    }
    
    /* Copy SSH hostname */
    if (ssh_hostname && strlen(ssh_hostname) > 0) {
    ...
    /* Generate detailed session ID */
    generate_session_id(logger->session_id, sizeof(logger->session_id),
                       logger->connection_id, logger->username, logger->remote_ip);
    ...
    guac_client_log(user->client, GUAC_LOG_INFO, 
                   "Command logging started - Connection ID: %s, User: %s, IP: %s, SSH Host: %s", 
                   logger->connection_id, logger->username, logger->remote_ip, logger->ssh_hostname);
...
void guac_ssh_command_logger_key(command_logger* logger, int keysym, int pressed) {
    ...
    if (keysym == 0xFF0D || keysym == 0x0D || keysym == 0x0A) {
        ...
        if (!is_empty) {
            ...
            if (logger->acl_config) {
                /* Get effective ACL rule - priority: user > connection > global */
                const char* guac_username = logger->client ? 
                    (logger->client->connection_id ? logger->client->connection_id : NULL) : NULL;
                
                guac_ssh_acl_rule* rule = guac_ssh_acl_get_rule(logger->acl_config,
                                                                 guac_username,
                                                                 logger->ssh_hostname,
                                                                 logger->username);
                
                if (rule) {
                    /* Check if command is allowed by ACL */
                    if (!guac_ssh_acl_check_command(rule, clean_cmd, logger->client)) {
                        is_restricted = true;
                        
                        /* Log to restricted commands file with same CSV format */
                        ...
                        syslog(LOG_WARNING, 
                               "ACL blocked command: user=%s ip=%s host=%s cmd=%s", 
                               logger->username, logger->remote_ip, 
            ...
            /* Check for dangerous commands (separate from ACL) */
            if (!is_restricted && is_dangerous_command(clean_cmd)) {
                /* Log to dangerous commands file with same CSV format */
                ...
                openlog("guacamole", LOG_PID | LOG_CONS, LOG_AUTH);
                syslog(LOG_WARNING, 
                       "Dangerous command executed: user=%s ip=%s cmd=%s", 
                       logger->username, logger->remote_ip, clean_cmd);
                closelog();
            }
        }
...
void guac_ssh_command_logger_flush(command_logger* logger) {
    ...
    fprintf(logger->log_file, "%s|%s|%s|%s|%s|%s|%s (incomplete)\n", 
            timestamp,
            logger->ssh_hostname,
            logger->connection_id,
            logger->session_id,
